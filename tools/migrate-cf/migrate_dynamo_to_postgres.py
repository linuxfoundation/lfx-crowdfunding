...
Key notes
---------
- initiatives.initiative_id = projectId (projects) or entityId (entities)
  stored as VARCHAR(255) — not cast to UUID; the column is a natural key.
- initiatives.id (UUID surrogate PK) is generated deterministically via
...
def migrate_organizations(cur, orgs: list, known_users: set) -> set:
    ...
    sql = """
        INSERT INTO organizations (id, organization_id, owner_id, name, avatar_url, status)
        VALUES (%s, %s, %s, %s, %s, %s)
    ...
    for o in orgs:
        ...
        rows.append((pg_id, pg_id, owner_id, o.get("name"), o.get("avatarUrl"), o.get("status")))
...
def migrate_initiatives(cur, entities: list, projects: list, known_users: set) -> set:
    ...
    for p in projects:
        ...
        initiative_rows.append((
            pg_id,                                       # id
            raw_initiative_id,                           # initiative_id VARCHAR(255)
            ...
            None,                                        # eventbrite_id  (entity only)
            None,                                        # application_url (entity only)
            _as_int(p.get("amountRaised")),              # amount_raised
        ...
        for cat_key, goal_name, sort_order in _PROJECT_GOAL_CATEGORIES:
            ...
            if amt > 0 or alloc or repo:
                goals_rows.append((
                    ...
                    raw_initiative_id, goal_name, amt, alloc, repo,
        ...
        if mentee:
            ...
            m_amt   = _as_int(m_budget.get("amount"))
            ...
            if m_amt > 0 or m_alloc or mentee.get("mentor") or mentee.get("terms") or mentee.get("skills"):
                goals_rows.append((
                    ...
                    raw_initiative_id, "mentee", m_amt, m_alloc, None,
                    None, None, None,
                    7,
                ))
            ...
            for idx, term in enumerate(mentee.get("terms") or []):
                if term:
                    program_info_terms_rows.append((
                        ...
                        raw_initiative_id, str(term), idx,
                    ))
        ...
        for idx, c in enumerate(pd.get("contributors") or []):
            ...
            contributors_rows.append((
                ...
                raw_initiative_id, c.get("name"), email,
            ))
        ...
        for idx, b in enumerate(pd.get("beneficiaries") or []):
            ...
            beneficiaries_rows.append((
                ...
                raw_initiative_id, b.get("name"), email,
            ))
        ...
        for cw in (pd.get("customWebsites") or []):
            ...
            if url:
                custom_websites_rows.append((
                    ...
                    raw_initiative_id, cw.get("name") or None, url,
                ))
    ...
    for e in entities:
        ...
        initiative_rows.append((
            ...
            e.get("eventbriteId") or None,                        # eventbrite_id
            e.get("applicationURL") or None,                      # application_url
            _as_int(e.get("amountRaised")),                       # amount_raised
        ...
        for idx, b in enumerate(e.get("beneficiaries") or []):
            ...
            beneficiaries_rows.append((
                _uuid5("beneficiary", raw_initiative_id, email or b.get("name", "") or str(idx)),
                raw_initiative_id, b.get("name") or None, email,
            ))
        ...
        if entity_type == "ostif":
            ...
            if isinstance(raw_detail, dict) and raw_detail:
                ostif_detail_rows.append((
                    ...
                    _as_int(raw_detail.get("totalBudget")),
...
def _insert_initiatives(cur, rows: list) -> None:
    ...
    sql = """
        INSERT INTO initiatives (
            id, initiative_id, initiative_type, source_dynamo_table,
            owner_id,
            ...
            eventbrite_id, application_url,
            amount_raised, accept_funding,
    ...
    sql = """
        INSERT INTO initiative_ostif_detail
            (initiative_id, monetization_strategy, current_security_strategy,
             license_type, total_budget, terms_conditions)
        VALUES (%s,%s,%s,%s,%s,%s)
        ON CONFLICT (initiative_id) DO UPDATE SET
...
def migrate_donations(
    ...
    sql = """
        INSERT INTO donations (
            ...
            created_on, current_amount_in_cents, payment_method, status, stripe_charge_id
        ) VALUES (
    ...
    for d in proj_donations:
        ...
        rows.append((
            ...
            _as_int(d.get("currentAmountInCents")),
            d.get("paymentmethod"),
            d.get("status"),
...
def migrate_subscriptions(
    ...
    for s in proj_subs:
        ...
        rows.append((
            ...
            _as_int(s.get("currentAmountInCents")),
            s.get("frequency"),
            s.get("status"),
