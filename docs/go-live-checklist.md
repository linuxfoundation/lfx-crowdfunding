...
### 4. Set `STRIPE_RETURN_URL` in all deployed environments

**Location:** `backend/cmd/initiatives-api/config.go` — `LoadConfig()`

**Why:** `STRIPE_RETURN_URL` is required — the service will not start without it.
...
### 5. Integration-test the 3DS subscription flow with Stripe test card `4000 0025 0000 3155`
...
### 6. Confirm `ledger-stats-sync` CronJob is running before go-live
...
### 7. Remove or disable `DISABLED_MOCK_LOCAL_PRINCIPAL` in all non-local environments
