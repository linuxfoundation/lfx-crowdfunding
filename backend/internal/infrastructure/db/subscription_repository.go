...
// scanSubscriptionWithInitiative scans a subscription row that includes the LEFT JOINed
// initiative name and logo_url (from listSubsForUser). Both fields use COALESCE so they
// are never NULL even when the referenced initiative is missing.
func scanSubscriptionWithInitiative(row scanner) (*models.Subscription, error) {
	s := &models.Subscription{}
	var (
		initiativeID, organizationID, category                        *string
		frequency, status                                             *string
...
func (r *SubscriptionRepository) listSubsForUser(ctx context.Context, userID string, filter models.SubscriptionFilter) ([]models.Subscription, *models.PaginationMeta, error) {
	...
	for rows.Next() {
		s, err := scanSubscriptionWithInitiative(rows)
		if err != nil {
