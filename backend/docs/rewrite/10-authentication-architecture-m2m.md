...
- The **Reimbursement Service** uses the same M2M pattern (Auth0 `client_credentials`) for its
  narrow read-only `/internal/v1/*` endpoints — planned, not yet implemented. Because RS reads
  CF-owned data with no logged-in user, it is a **pure service-to-service** caller: it presents
  only the M2M token and sends no user-identity headers.

This unifies the Go API's trust model: every caller is a known, explicitly allowlisted M2M
client validated through one JWT pipeline. User-acting callers (Nuxt BFF, Self Serve) add
`X-Username`/`X-User-Email`; the service-only caller (RS) does not.
...
| **Reimbursement Service** | Trusted server (Lambda) | Calls `/internal/v1/*` via M2M (service-only, no user identity); planned |
...
graph TD
    ...
    RS -->|"client_credentials grant\nRS_M2M_CLIENT_ID/SECRET\naudience: CF /api/"| Auth0
    Auth0 -->|"M2M token (cached)"| RS
    RS -->|"Bearer M2M token\n(no user headers)\nGET /internal/v1/*"| CFAPI
...
The Reimbursement Service (RS) is an AWS Lambda. It authenticates to CF using the **same M2M
pattern** as the other callers: an Auth0 `client_credentials` token for the CF API audience
(`/api/`). RS reads CF-owned data with no logged-in user, so it is a **pure service-to-service**
caller — it presents only the M2M token and sends **no** `X-Username`/`X-User-Email` headers.
...
sequenceDiagram
    ...
    Note over RS,Auth0: At cold start (or on token expiry)
    RS->>Auth0: POST /oauth/token<br/>grant_type: client_credentials<br/>client_id: RS_M2M_CLIENT_ID<br/>client_secret: RS_M2M_CLIENT_SECRET<br/>audience: https://crowdfunding.{env}.lfx.dev/api/
    Auth0->>RS: M2M access token (~24 h, cached)

    RS->>API: GET /internal/v1/initiatives?slug={slug}<br/>Authorization: Bearer {M2M token}<br/>(no user-identity headers)
    API->>API: validate JWT · isM2MToken · azp ∈ AUTHORIZED_CLIENTS
...
The `/internal/v1/*` endpoints stay separate from the public `/v1/*` routes because they expose
internal fields (`owner_id`, `email`) not present on public responses. They are protected by the
same M2M middleware — the RS client ID must appear in `AUTHORIZED_CLIENTS`. No shared secret is
used.
...
| **M2M required (service-only)** | `/internal/v1/*` *(planned)* | M2M JWT from allowlisted client; no user-identity headers |
...
| Mechanism | Scope |
|---|---|
| **Auth0 JWT validation** (JWKS, RS256) | Every M2M-protected route (`/v1/*` and `/internal/v1/*`). Single resource server `lfx_crowdfunding_api` (`/api/`) — same audience for all M2M clients (Nuxt BFF, Self Serve, RS). |
| **`AUTHORIZED_CLIENTS` allowlist** | Restricts which Auth0 client IDs may call the API (Nuxt BFF, Self Serve, RS). Also gates `X-Username`/`X-User-Email` header trust — only allowlisted user-acting clients set them; RS sets neither. |
...
Three `auth0_client_grant` resources are needed — one each for the Nuxt BFF, Self Serve, and
Reimbursement Service M2M clients. All target the existing `lfx_crowdfunding_api` resource
server. No new resource server is needed.
...
# Reimbursement Service M2M client → CF API (planned)
resource "auth0_client_grant" "reimbursement_crowdfunding" {
  client_id  = auth0_client.lfx_reimbursement_m2m.id
  audience   = auth0_resource_server.lfx_crowdfunding_api.identifier
  scopes     = ["access:api"]
  depends_on = [auth0_resource_server_scopes.lfx_crowdfunding_api]
}
...
| **CF Go API** (`server.go`) | Add `POST /v1/users/sync` route (M2M-authenticated; upserts user row). Add `/internal/v1/*` routes under M2M `Middleware` (no `X-Internal-Token` check). |
...
| **Auth0 Terraform** | `auth0_client_grant` for Nuxt BFF, Self Serve, and RS M2M clients (see above). |
| **ArgoCD values** | `AUTHORIZED_CLIENTS`: add Nuxt BFF, Self Serve, and RS M2M `client_id`s. Add `NUXT_M2M_CLIENT_ID`/`NUXT_M2M_CLIENT_SECRET` to CF frontend values; RS M2M creds to RS deployment. |
...
| **Reimbursement Service** | Mint M2M `client_credentials` token for CF `/api/` audience; attach as Bearer on `/internal/v1/*` calls. Removes prior `X-Internal-Token` shared-secret approach. |
...
| `AUTHORIZED_CLIENTS` | Comma-separated allowlisted Auth0 client IDs (Nuxt BFF + Self Serve + RS M2M) | injected via ESO |
...
### Reimbursement Service *(planned)*

| Env var | Purpose |
|---|---|
| `RS_M2M_CLIENT_ID` | Auth0 M2M client ID for CF `/internal/v1/*` calls |
| `RS_M2M_CLIENT_SECRET` | Auth0 M2M client secret |
| `RS_M2M_ISSUER_BASE_URL` | Auth0 token endpoint base URL |
| `CROWDFUNDING_API_BASE_URL` | CF API base URL |
| `CROWDFUNDING_API_AUDIENCE` | CF API audience (`https://crowdfunding.{env}.lfx.dev/api/`) |
...
- [`04-target-architecture.md`](04-target-architecture.md) — overall target architecture
  including Reimbursement Service internal endpoints and Auth0 tenant topology. Note: `04`
  describes the RS `/internal/v1/*` endpoints with an `X-Internal-Token` shared secret; this
  document supersedes that with the M2M approach above.
