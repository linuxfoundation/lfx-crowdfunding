...
The `@fgadoc:` comments below illustrate the *target* state only. They are generated by the
platform's fgadoc skill from deployed Heimdall RuleSets and **must not be hand-written into the
`lfx-v2-helm` model PR** — see "Architecture feedback" below.
...
- **The permission-matrix mechanism already exists upstream.** A platform skill reads deployed
  ...
  as `@fgadoc:` comments; `PERMISSIONS.md` (the permission → task fan-out) is generated from those
  comments. CF doesn't need its own version of this — once this model merges upstream, the same
  skill covers it.
- **`@fgadoc:jtbd` lines are never hand-authored in a model PR.** They're emitted by that skill,
  and only once a RuleSet actually references the relation/permission — a RuleSet can't reference
  something that doesn't exist yet, so a jtbd comment can never legitimately land in the same
  commit that adds the relation. Concretely: **relation lands first → a RuleSet referencing it
  lands second → the fgadoc skill emits the jtbd comment third.** Never the same commit. It's
  fine to use jtbd lines in *this design doc* to visualize the target model (as above) — just not
  in the actual `lfx-v2-helm` model PR. The instruction used to live in `model.fga` itself; after
  a file split it's now in
  [`templates/openfga/model.yaml#L24-L33`](https://github.com/linuxfoundation/lfx-v2-helm/blob/main/charts/lfx-platform/templates/openfga/model.yaml#L24-L33) —
  easy to miss the move.
