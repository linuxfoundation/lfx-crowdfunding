...
describe('require-auth middleware', () => {
  ...
  describe('POST /api/presigned-url', () => {
    it('throws 401 when no token is present', () => {
      const event = makeEvent('POST', '/api/presigned-url');
      expect(() => (requireAuth as (e: unknown) => void)(event)).toThrow();
      expect(mockCreateError).toHaveBeenCalledWith({
        statusCode: 401,
        statusMessage: 'Authentication required',
    ...
    it('passes through when a valid token is present', () => {
      const event = makeEvent('POST', '/api/presigned-url', 'eyJtoken');
      expect(() => (requireAuth as (e: unknown) => void)(event)).not.toThrow();
    });

    it('passes through for GET — wrong method is not protected', () => {
      const event = makeEvent('GET', '/api/presigned-url');
      expect(() => (requireAuth as (e: unknown) => void)(event)).not.toThrow();
      expect(mockCreateError).not.toHaveBeenCalled();
    });
  });
  ...
  describe('POST /api/fundraise', () => {
    it('throws 401 when no token is present', () => {
      const event = makeEvent('POST', '/api/fundraise');
      expect(() => (requireAuth as (e: unknown) => void)(event)).toThrow();
      expect(mockCreateError).toHaveBeenCalledWith({
        statusCode: 401,
        statusMessage: 'Authentication required',
    ...
    it('passes through when a valid token is present', () => {
      const event = makeEvent('POST', '/api/fundraise', 'eyJtoken');
      expect(() => (requireAuth as (e: unknown) => void)(event)).not.toThrow();
    });

    it('passes through for GET — wrong method is not protected', () => {
      const event = makeEvent('GET', '/api/fundraise');
      expect(() => (requireAuth as (e: unknown) => void)(event)).not.toThrow();
      expect(mockCreateError).not.toHaveBeenCalled();
    });
  });
