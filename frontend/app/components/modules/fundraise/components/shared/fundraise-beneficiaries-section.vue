  ...
  <div class="border border-neutral-200 rounded-xl p-6">
    <div class="flex flex-col gap-5">
      ...
      <div class="flex flex-col">
        <div
          v-for="(beneficiary, index) in modelValue"
          :key="index"
          class="flex items-center gap-3 py-4"
          :class="index < modelValue.length - 1 ? 'border-b border-neutral-200' : ''"
        >
          <lfx-input
...
import type { Beneficiary } from '~/types/fundraise.types';
...
const emit = defineEmits<{
  (e: 'update:modelValue', value: Beneficiary[]): void;
}>();
...
const updateBeneficiary = (index: number, field: keyof Beneficiary, value: string) => {
  emit(
    'update:modelValue',
    props.modelValue.map((b, i) => (i === index ? { ...b, [field]: value } : b)),
  );
};
</script>
...
<script lang="ts">
export default {
  name: 'FundraiseBeneficiariesSection',
};
</script>
